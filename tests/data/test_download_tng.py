...
# ---------------------------------------------------------------------------
# clearml_task integration
# ---------------------------------------------------------------------------

from unittest.mock import MagicMock, patch


def test_download_tng_data_calls_register_when_task_provided(tmp_path):
    """register_or_get_dataset is called after download when clearml_task is set."""
    raw_dir = str(tmp_path / "raw")
    processed_dir = str(tmp_path / "processed")
    mock_task = MagicMock()

    with patch("src.data.download_tng.extract_tng_urls", return_value=[]):
        with patch("src.data.download_tng.register_or_get_dataset") as mock_register:
            from src.data.download_tng import download_tng_data
            download_tng_data(
                raw_dir=raw_dir,
