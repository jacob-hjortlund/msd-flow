...
OmegaConf.register_new_resolver(
    ...
    lambda start, count: [start + i for i in range(count)]
)
...
def extract_tng_urls(
    ...
    for vId, snap in tqdm(combos):
        ...
        try:
            ...
            log.warning(f"Skipping version {vId}, snapID {snap} - API returned {err.response.status_code}")
...
def download_tng_fits_file(
    ...
    for attempt in range(max_retries):
        try:
            ...
            sleep_time = timeout_base ** attempt
            log.warning(f"Connection dropped for {unique_filename}. Retrying in {sleep_time}s...")
            time.sleep(sleep_time)
...
def main(cfg: DictConfig):
    ...
    with ThreadPoolExecutor(max_workers=dl_cfg.max_workers) as executor:
        future_to_url = {
            executor.submit(
                download_tng_fits_file, url, dl_cfg.raw_dir, headers 
            ): url for url in urls
        }
