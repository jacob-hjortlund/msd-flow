...
# Defer JAX GPU initialization until after ClearML's os.fork().
# ClearML's BackgroundMonitor forks the process for reporting, which
# corrupts any pre-existing CUDA context.  By restricting JAX to CPU
# during module import we guarantee CUDA is untouched when the fork
# happens.  _reinit_jax_gpu() re-enables the GPU afterwards.
...
def _reinit_jax_gpu():
    """Re-enable GPU after ClearML's ``os.fork()``.

    Clears JAX's cached backend list so the next device-touching
    operation discovers the GPU cleanly, after the fork is complete.
    """
    ...
    jax.clear_caches()
    log.info("JAX devices after GPU reinit: %s", jax.devices())
...
def main(cfg: DictConfig):
    ...
    # Now that the fork is done, re-enable GPU.
    _reinit_jax_gpu()
